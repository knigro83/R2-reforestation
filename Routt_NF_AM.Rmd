...
routt_9000_bz <- co_breedingzones %>% filter(SEED_COL_Z %in% c(26,29,55) & r2elev_z10==9000)
...
breeding_zones_10000 <- project(routt_10000_bz,"+proj=longlat +ellps=WGS84 +no_defs")
breeding_zones_9000 <- project(routt_9000_bz,"+proj=longlat +ellps=WGS84 +no_defs")
...
breeding_zones_10000 <- breeding_zones_10000[breeding_zones_10000$ZONE_NO!="0",]
breeding_zones_10000 <- breeding_zones_10000 %>% 
  mutate(breeding_z = seq(1,nrow(breeding_zones_10000),1))

breeding_zones_9000 <- breeding_zones_9000[breeding_zones_9000$ZONE_NO!="0",]
breeding_zones_9000 <- breeding_zones_9000 %>% 
  mutate(breeding_z = seq(1,nrow(breeding_zones_9000),1))
...
# loop through each climate variable in "vars"
  for (i in 1:nlyr(all_climate)) {
    ...
    # make a temp dataframe copying the data for each zone
    bz_df <- as.data.frame(breeding_zones_10000)
    # append climate summary data
    bz_df$zone_max <- zmax
    bz_df$zone_min <- zmin
    bz_df$zone_mean <- zmean
    ...
    bz_df$clim_var <- variable
    # add to master dataframe
    full_df_10000 <- rbind(full_df_10000,bz_df)
  }
...
# loop through each climate variable in "vars"
  for (i in 1:nlyr(all_climate)) {
    ...
    # make a temp dataframe copying the data for each zone
    bz_df <- as.data.frame(breeding_zones_9000)
    # append climate summary data
    bz_df$zone_max <- zmax
    bz_df$zone_min <- zmin
    bz_df$zone_mean <- zmean
    ...
    bz_df$clim_var <- variable
    # add to master dataframe
    full_df_9000 <- rbind(full_df_9000,bz_df)
  }
...
all_climate_wide <- routt_climate_stats %>% 
  dplyr::select(c(ZONE_NO, elev, period, climvar, zone_center)) %>% 
...
ggplot(clim_allspecies, aes(x=MCMT, y=SHM))+
  geom_density_2d_filled(contour_var = "ndensity", n=20)+
  facet_grid(elev~species, scales="free")+
  geom_point(data=all_climate_wide, aes(x=MCMT, y=SHM, color=period, shape=factor(ZONE_NO)), size=3)+
